...
- База: `price_check.db` (SQLite), 11 юзеров, **36 активных товаров**
...
## Таблицы магазинов
- `known_shops` (99 доменов) — белый список проверенных магазинов.
...
## Что сделано за сессию (2026-07-23)
- ✅ **motodnepr.ua** — разобрался, почему не читает. В базе лежала ссылка на страницу оформления заказа (`index.php?route=extension/quickcheckout/checkout`), не на товар. Парсер motodnepr.ua читает отлично (JSON-LD + meta + селекторы). Удалил мусорный item #236, добавил правильную ссылку как #237 (Генератор Hyundai HHY 10050Si — 88 725 грн), зарегистрировал в `known_shops`.
- ✅ **Юзеры (сводка)**: 11 юзеров, 36 активных товаров. Edj — 30, Alexei Borisov — 4, Mykhailo — 1, Alexvit — 1. Остальные 7 аппрувнуты, но пусты.
